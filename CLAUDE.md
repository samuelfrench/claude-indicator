...
- **LocalTokensRow**: Today (since local midnight) and all-time local-model token volume from the read-only OpenCode SQLite ledger (`providerID = "ollama"`), with message counts, ledger start date, and latest local model. Scoped to OpenCode on purpose and labelled `LOCAL (OPENCODE)` in the UI — the clawd-bot runner drives ollama through aider and is not recorded in that ledger.
